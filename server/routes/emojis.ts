 ...
 *           type: boolean
 *           default: false
 *         description: Whether to include soft-deleted emojis
 *       - in: query
 *         name: page
 *         schema:
 *           type: integer
...
router.get('/', isAuthenticated, async (req: Request, res: Response) => {
  try {
    const { includeDeleted, page, limit } = listEmojisSchema.parse(req.query);
    const offset = (page - 1) * limit;

    // Only filter out deleted emojis if includeDeleted is false
    const where = includeDeleted ? undefined : eq(emojis.deleted, false);
    ...
    // Get total count, considering the includeDeleted flag
    ...
    // Return array of emojis as specified in OpenAPI spec
