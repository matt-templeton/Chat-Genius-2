...
export const channels = pgTable(
  ...
  (table) => ({
    ...
    workspaceNameIdx: uniqueIndex("idx_channels_workspace_name").on(
      table.workspaceId,
...
export const files = pgTable(
  ...
  (table) => ({
    ...
    notDeletedIdx: index("idx_files_not_deleted")
      .on(table.userId, table.fileId)
      .where(sql`${table.softDeleted} = false`),
  }),
);
